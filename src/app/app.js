...
async function createVideoDraft() {
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(
        `Failed to create video draft: ${data.error}`,
      );
    }
...
async function signup() {
  ...
  try {
    ...
    if (!res.ok) {
      const data = await res.json();
      throw new Error(
        `Failed to create user: ${data.error}`,
      );
    }
...
async function uploadThumbnail(videoID) {
  ...
  try {
    ...
    if (!res.ok) {
      const data = await res.json();
      throw new Error(
        `Failed to upload thumbnail. Error: ${data.error}`,
      );
    }
...
async function getVideos() {
  try {
    ...
    if (!res.ok) {
      const data = await res.json();
      throw new Error(
        `Failed to get videos. Error: ${data.error}`,
      );
    }
...
function viewVideo(video) {
  currentVideo = video;
  document.getElementById('video-display').style.display =
    'block';
  document.getElementById(
    'video-title-display',
